...
def dim_driver_view():
  df = spark.readStream.table("silver_obt")
  df = df.select("driver_id", "driver_name", "driver_phone", "driver_license")
...
def dim_vehicle_view():
  df = spark.readStream.table("silver_obt")
  df = df.select("vehicle_id", "vehicle_make_id", "vehicle_type_id", "vehicle_model", "vehicle_color", "license_plate", "vehicle_make",
                 "vehicle_type")
...
def dim_payment_view():
    ...
    df = df.select("payment_method_id","total_fare","base_rate")
...
def dim_location_view():
    df = spark.readStream.table("uber.bronze.silver_obt")
    df = df.select("pickup_city_id","pickup_address", "pickup_location_id", "booking_timestamp")
